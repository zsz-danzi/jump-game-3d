...
var Game = function(){
  ...
  function initStats() {
	...
	return {
      ...
      factor = false;
			// 清空plane所有物体
			let len = this.plane.children.length;
			for (let i = 0; i < len; i++) {
					...
					if( man.ready_flag){
						...
						if( !man.first_flag1 ){
							...
							for(var i = 0; i < block_arr.length; i++){
								var block = block_arr[i];

								if( _this.collision(man, block)){
									block_id = i+1 ; //与第几个碰撞了

									console.log('正常碰撞',  block_id, block_index, block_arr)
						...
						console.log('man&block', man, block_arr)
						...
						if(!first_flag){ //生成下一块
							...
							// console.log(distance, block_index)
